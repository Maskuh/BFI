...
class heart(commands.Cog):
    ...
    async def addsubserver(self, interaction: discord.Interaction, main_server_id: str, sub_server_id: str, name: str):
        if not (await self.is_site_admin(str(interaction.user.id)) or await self.is_superadmin(str(interaction.user.id))):
            await interaction.response.send_message("You do not have permission to use this command.")
            return

        main_server = await self.db.servers.find_one({"server_id": main_server_id, "type": "main"})
        if not main_server:
            await interaction.response.send_message(f"Main server with ID {main_server_id} not found.")
        else:
            ...
            await self.db.servers.update_one({"server_id": main_server_id}, {"$push": {"linked_servers": sub_server_id}})
            await interaction.response.send_message(f"Sub-server {name} added under main server {main_server_id}.")

    @app_commands.command(description="views sub servers in a main server")
    async def viewsubservers(self, interaction: discord.Interaction, main_server_id: str):
        if not (await self.is_site_admin(str(interaction.user.id)) or await self.is_superadmin(str(interaction.user.id))):
            await interaction.response.send_message("You do not have permission to use this command.")
            return
        ...
        main_server = await self.db.servers.find_one({"server_id": main_server_id, "type": "main"})
        if not main_server:
            await interaction.response.send_message(f"No main server found with ID {main_server_id}.")
        else:
            sub_servers = await self.db.servers.find({"server_id": {"$in": main_server["linked_servers"]}}).to_list(length=100)
            sub_server_names = [server["name"] for server in sub_servers]
            await interaction.response.send_message(f"Sub-servers linked to {main_server['name']}: {', '.join(sub_server_names)}")
    ...
    async def viewmainservers(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"Main servers: {', '.join(main_server_names)}")
    ...
    async def autofollow(self, interaction: discord.Interaction):
        ...
        await interaction.send(f"Newsletter and bot logs channels created: {newsletter_channel.mention}, {bot_logs_channel.mention}")
    # Ensure the bot has necessary permissions in the target channel
        if not interaction.channel.permissions_for(interaction.guild.me).manage_webhooks:
            await interaction.response.send_message("I need the 'Manage Webhooks' permission to follow the news channel.")
            return
    ...
    # Make the request
        ...
        if response.status_code == 200 and responses.status_code == 200:
            await interaction.response.send_message("Successfully followed the news channel! ")
        else:
            await interaction.response.send_message(f"Failed to follow the channel: {response.status_code} - {response.text}")

    @app_commands.command(description="Adds a main server to the list")
    async def addmainserver(self, interaction: discord.Interaction, server_id: str, name: str):
        ...
        if server:
            ...
            await interaction.response.send_message(f"Main server {name} added with ID {server_id}.")
    ...
    async def siteadmin(self, interaction: discord.Interaction, user_id: str):
        ...
        if not user:
            ...
            await self.db.sitea.update_one({"user_id": user_id}, {"$set": {"site_admin": True}})
    ...
    async def rsiteadmin(self, interaction: discord.Interaction, user_id: str):
        ...
        result = await self.db.sitea.update_one({"user_id": user_id}, {"$set": {"site_admin": False}})
        if result.matched_count == 0:
            await interaction.response.send_message(f"User {user_id} not found.")
        else:
            await interaction.response.send_message(f"Site admin privileges removed from user {user_id}.")
    ...
    async def addsuperadmin(self, interaction: discord.Interaction, user_id: str):
        if str(interaction.user.id) != "786788350160797706":
            await interaction.response.send_message("Only the root Superadmin can use this command.")
            return
        ...
        if user:
            ...
            await interaction.response.send_message(f"User {user_id} has been added as a Superadmin.")
...
class ReportModal(ui.Modal, title='Report'):
    ...
    reason = ui.TextInput(label='Reason', style=discord.TextStyle.short,
                          placeholder="Why you are reporting the bug", required=True)
    bug = ui.TextInput(label='Bug Description', style=discord.TextStyle.paragraph,
                       placeholder="Describe what the bot did that it shouldn't have", required=True)
