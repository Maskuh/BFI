...
async def on_ready():
  try:
    await bot.load_extension("cogs.gang")
    await bot.load_extension("cogs.gamble")
    await bot.tree.sync()
  except Exception as e:
    print(f"The bot has reported the error \n ({e})")
  await bot.change_presence(status= discord.Status.online, activity=discord.Activity(type=discord.ActivityType.watching, name="How To Be Useful"))
